-- Update get_event_details_optimized function to include meetup_spot for participants only
CREATE OR REPLACE FUNCTION public.get_event_details_optimized(p_event_id uuid, p_user_id uuid DEFAULT NULL::uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Check if user is participant or host for meetup_spot access
  ...
  -- Include meetup_spot only for participants and hosts
  SELECT jsonb_build_object(
    ...
    'location', e.location,
    'meetup_spot', CASE 
      WHEN (user_is_participant OR user_is_host) THEN e.meetup_spot
      ELSE NULL 
    END,
    'date', e.date,
    'time', e.time,
  ...
  IF p_user_id IS NOT NULL THEN
    SELECT jsonb_build_object(
      'isAttending', user_is_participant,
      'isSaved', EXISTS(
