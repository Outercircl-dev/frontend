-- Security Enhancement: Strengthen RLS for Sensitive Data Tables (Corrected)
...
-- Apply audit trigger to profiles_sensitive table
...
-- Apply audit trigger to payment_metadata table
DROP TRIGGER IF EXISTS audit_payment_metadata_trigger ON public.payment_metadata;
CREATE TRIGGER audit_payment_metadata_trigger
...
-- 3. Strengthen existing RLS policies for payment_metadata table
DROP POLICY IF EXISTS "payment_metadata_secure_owner" ON public.payment_metadata;
...
-- 4. Add missing RLS for invitations_admin_secure table (if it's a table, not view)
...
BEGIN
  IF EXISTS (
    ...
    -- Create new ultra-secure policy
    EXECUTE 'CREATE POLICY "invitations_admin_ultra_secure" ON public.invitations_admin_secure
      FOR ALL
      TO authenticated
      USING (
        auth.uid() IS NOT NULL 
...
BEGIN
  ...
  IF p_table_name IN ('payment_metadata') THEN
    RETURN auth.uid() = p_record_id;
  END IF;
...
ON public.payment_metadata (user_id, created_at, security_level);
...
'Ultra-secure RLS policy for payment data - requires authenticated user, matching user_id, and valid JWT audience';
...
COMMENT ON TRIGGER audit_payment_metadata_trigger ON public.payment_metadata IS 
'Automatic audit logging for all operations on sensitive payment data';