...
CREATE POLICY "profiles_sensitive_owner_authenticated_only" 
...
CREATE POLICY "payment_metadata_owner_session_validated" 
...
USING (
  ...
  AND auth.jwt() IS NOT NULL
  AND (auth.jwt() ->> 'aud') = 'authenticated'
  AND EXTRACT(EPOCH FROM (NOW() - auth.jwt()::timestamptz)) < 3600 -- 1 hour session limit
)
WITH CHECK (
  auth.uid() IS NOT NULL 
  AND auth.uid() = user_id
...
-- Update sensitive table policies to include account lockdown check
DROP POLICY IF EXISTS "profiles_sensitive_owner_authenticated_only" ON public.profiles_sensitive;
DROP POLICY IF EXISTS "payment_metadata_owner_session_validated" ON public.payment_metadata;
...
USING (
  ...
  AND auth.jwt() IS NOT NULL
  AND (auth.jwt() ->> 'aud') = 'authenticated'
  AND NOT public.check_account_security_status(auth.uid())
)
WITH CHECK (
  auth.uid() IS NOT NULL 
  AND auth.uid() = id
...
USING (
  ...
  AND auth.jwt() IS NOT NULL
  AND (auth.jwt() ->> 'aud') = 'authenticated'
  AND EXTRACT(EPOCH FROM (NOW() - auth.jwt()::timestamptz)) < 3600
  AND NOT public.check_account_security_status(auth.uid())
)
WITH CHECK (
  auth.uid() IS NOT NULL 
