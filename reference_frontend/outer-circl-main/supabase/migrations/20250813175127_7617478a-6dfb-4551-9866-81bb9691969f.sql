-- SECURITY FIX: Protect Customer Email Addresses and Payment Data - Final Fix
...
USING (
  ...
  validate_sensitive_access('invitations', 'admin_access')
)
WITH CHECK (
  auth.uid() IS NOT NULL AND 
  is_subscription_admin(subscription_id, auth.uid()) AND
  validate_sensitive_access('invitations', 'admin_write')
);
...
USING (
  ...
  auth.uid() = id AND 
  id IS NOT NULL AND
  validate_sensitive_access('profiles_sensitive', 'sensitive_access')
)
WITH CHECK (
  auth.uid() IS NOT NULL AND 
  auth.uid() = id AND 
  id IS NOT NULL AND
  validate_sensitive_access('profiles_sensitive', 'sensitive_write')
);
...
-- Step 10: Create alert function for suspicious access patterns
CREATE OR REPLACE FUNCTION public.check_suspicious_access_patterns()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = 'public'
...
BEGIN
  -- Check for excessive sensitive data access in the last hour
  SELECT COUNT(DISTINCT user_id) INTO suspicious_count
  FROM public.security_audit_enhanced
  WHERE timestamp > NOW() - INTERVAL '1 hour'
  AND resource_type IN ('invitations', 'profiles_sensitive')
  GROUP BY user_id
  HAVING COUNT(*) > 50;
  
  -- Log alert if suspicious activity detected
  IF suspicious_count > 0 THEN
    PERFORM log_sensitive_access(
      ...
      null,
      jsonb_build_object(
        'alert_type', 'excessive_sensitive_access',
        'affected_users', suspicious_count,
        'timestamp', NOW()
      )
    );
  END IF;
...
SELECT 'SECURITY ENHANCEMENT COMPLETED: Enhanced protection for customer emails and payment data with comprehensive audit logging' as status;