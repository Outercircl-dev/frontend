...
DROP FUNCTION IF EXISTS public.discover_events_secure(uuid);
DROP FUNCTION IF EXISTS public.discover_events_secure();
...
BEGIN
  ...
  IF NOT public.check_rate_limit(p_user_id, NULL, 'event_discovery', 100, 60) THEN
    -- Log suspicious activity (only if log function exists)
  ...
  -- Log legitimate discovery for security monitoring (only if log function exists)
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'log_security_event') THEN
    PERFORM public.log_security_event(
      'event_discovery',
      'events',
