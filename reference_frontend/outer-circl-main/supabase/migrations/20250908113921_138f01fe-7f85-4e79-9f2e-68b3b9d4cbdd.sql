...
-- Drop problematic functions that query auth.users directly
...
-- Drop all existing RLS policies on invitations table
...
-- Drop all existing RLS policies on payment_metadata table  
...
-- Create trigger to automatically log payment access (INSERT/UPDATE/DELETE only)
...
-- Create trigger to log invitation access (INSERT/UPDATE/DELETE only)
...
-- Update profiles_sensitive policy to be even more secure
