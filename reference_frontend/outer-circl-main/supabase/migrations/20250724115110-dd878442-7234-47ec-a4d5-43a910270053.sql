...
-- Add a timestamp column to track when the event was completed
...
DROP POLICY IF EXISTS "Users can rate other event participants within 24 hours" ON public.user_ratings;
...
WITH CHECK (
  ...
  AND EXISTS (
    ...
    AND (
      e.completed_at IS NULL OR 
      e.completed_at > NOW() - INTERVAL '24 hours'
    )
  )
);
