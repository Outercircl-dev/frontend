-- Enhanced Security Measures for Sensitive Data Tables
...
-- 4. Add triggers for comprehensive audit logging on sensitive tables
...
CREATE TRIGGER audit_payment_metadata_access
...
-- 5. Create enhanced security policies with additional constraints
-- These policies add extra validation layers

-- Enhanced payment metadata policies with stricter validation
DROP POLICY IF EXISTS "payment_metadata_enhanced_owner_select" ON public.payment_metadata;
CREATE POLICY "payment_metadata_enhanced_owner_select" ON public.payment_metadata
FOR SELECT 
USING (
  ...
  AND EXISTS (
    ...
    AND u.created_at < now() - interval '1 hour' -- Account must be at least 1 hour old
  )
);

-- Enhanced profiles sensitive policies with additional security checks
DROP POLICY IF EXISTS "profiles_sensitive_enhanced_owner_select" ON public.profiles_sensitive;
CREATE POLICY "profiles_sensitive_enhanced_owner_select" ON public.profiles_sensitive
FOR SELECT 
USING (
  auth.uid() IS NOT NULL 
  AND auth.uid() = id 
...
-- 6. Create function to check if user can access sensitive data
...
-- 7. Create function to monitor suspicious access patterns
...
-- 9. Create function to validate encrypted data integrity
...
-- 10. Grant minimal necessary permissions
-- Revoke any unnecessary permissions (safety measure)
...
-- Restrict access to monitoring view
ALTER VIEW public.security_sensitive_data_monitor OWNER TO postgres;
