-- CRITICAL SECURITY FIX: Prevent Event Data Scraping by Competitors (CORRECTED)
...
-- 2. Create a much more restrictive and secure function for event access
...
-- 3. Replace the old user_can_view_event function with secure version
...
AS $$
BEGIN
  RETURN public.user_can_view_event_secure(event_id, user_id);
END;
$$;

-- 4. Create a discovery function that prevents mass scraping but allows legitimate browsing
...
-- 5. Create a secure events view that protects sensitive data
...
SELECT 
  ...
  END as coordinates
FROM public.events e
WHERE public.user_can_view_event_secure(e.id, auth.uid());
...
-- 7. Create summary of security improvements
SELECT 
