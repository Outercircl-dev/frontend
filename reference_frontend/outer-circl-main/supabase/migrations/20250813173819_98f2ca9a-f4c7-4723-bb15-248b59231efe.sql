...
CREATE POLICY "block_all_other_invitation_access" ON public.invitations
FOR ALL
USING (false)
WITH CHECK (false);
...
-- Step 5: Add rate limiting for sensitive data access
...
-- Add rate limiting to profiles_sensitive access
CREATE OR REPLACE FUNCTION public.check_sensitive_profile_access()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $$
BEGIN
  -- Rate limit sensitive profile access
  IF TG_OP = 'SELECT' AND NOT public.check_profile_access_rate_limit(auth.uid(), 'sensitive_profile_access') THEN
    RAISE EXCEPTION 'Rate limit exceeded for sensitive profile access';
  END IF;
...
DROP TRIGGER IF EXISTS rate_limit_profiles_sensitive ON public.profiles_sensitive;
CREATE TRIGGER rate_limit_profiles_sensitive
  BEFORE SELECT ON public.profiles_sensitive
  FOR EACH ROW EXECUTE FUNCTION public.check_sensitive_profile_access();
