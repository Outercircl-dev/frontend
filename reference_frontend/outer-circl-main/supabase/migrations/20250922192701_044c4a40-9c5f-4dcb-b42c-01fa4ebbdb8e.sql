-- Fixed Enhanced Security Migration for Critical Issues
...
-- 5. Create triggers for enhanced logging (fixed - only for INSERT/UPDATE/DELETE)
...
-- 6. Enhanced RLS policy for invitations to prevent email harvesting
...
-- More restrictive invitation access policy
...
USING (
  auth.uid() IS NOT NULL
  AND (
    -- Admin access to their own subscription invitations only
    (EXISTS (
    ...
    OR
    -- User can only see their own pending invitations by matching email (highly restricted)
    (
      auth.email() = email
      AND status = 'pending'
      AND expires_at > now()
      AND created_at > now() - interval '7 days' -- Only recent invitations
    )
  )
...
-- Secure invitation creation policy
CREATE POLICY "invitations_secure_creation"
ON public.invitations
FOR INSERT
WITH CHECK (
  auth.uid() IS NOT NULL
  ...
  -- Rate limit invitation creation (10 per hour)
  AND (
...
-- Secure invitation update policy
...
-- 7. Create a secure view for invitations that masks sensitive data
...
SELECT 
  ...
  updated_at,
  status,
  -- Mask email addresses for security - only show partial email
  CASE 
...
-- 8. Create security monitoring function for admins
...
BEGIN
  ...
  WITH breach_analysis AS (
    ...
    HAVING COUNT(*) > 50 -- Lower threshold for monitoring
    
    UNION ALL
    
    SELECT 
      'Suspicious Invitation Access' as breach_type,
      COUNT(DISTINCT user_id) as user_count,
      MAX(timestamp) as last_seen,
      'MEDIUM' as risk
    FROM public.security_audit_enhanced
...
-- 10. Add security documentation comments
