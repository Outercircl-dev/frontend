-- Security Fix: Secure Payment Information Access
...
AS $$
BEGIN
  -- Log all payment data access for security monitoring
...
AFTER SELECT OR UPDATE OR INSERT ON public.membership_subscriptions
FOR EACH ROW
EXECUTE FUNCTION public.log_payment_access();

-- 6. Add additional security comments
COMMENT ON TABLE public.membership_subscriptions IS 'Payment-sensitive data: Contains Stripe customer and subscription IDs. Access strictly limited to subscription administrators only.';
