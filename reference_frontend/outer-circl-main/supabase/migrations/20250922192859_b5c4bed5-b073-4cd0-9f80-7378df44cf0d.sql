-- Simplified Enhanced Security Migration for Critical Issues
-- Addresses payment data and PII exposure vulnerabilities

-- 1. Create additional rate limiting function for payment operations
...
-- 2. Enhanced RLS policy for profiles_sensitive
...
-- 3. Enhanced RLS policy for payment_metadata
...
USING (
  ...
  AND check_payment_access_rate_limit(auth.uid())
  -- Business hours restriction (6 AM to 10 PM UTC)
...
-- 4. Enhanced RLS policies for invitations to prevent email harvesting
...
-- Restrictive invitation SELECT policy
...
-- Secure invitation INSERT policy
...
-- 5. Create a secure view for invitations with masked emails
...
-- 6. Create security monitoring function for admins
