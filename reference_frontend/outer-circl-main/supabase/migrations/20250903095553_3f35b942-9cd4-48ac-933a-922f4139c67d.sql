...
DROP TRIGGER IF EXISTS notify_saved_participants ON public.event_participants;  
DROP TRIGGER IF EXISTS track_participation ON public.event_participants;

-- Now drop and recreate the functions
DROP FUNCTION IF EXISTS public.notify_event_participants() CASCADE;
DROP FUNCTION IF EXISTS public.notify_saved_event_participants() CASCADE;
DROP FUNCTION IF EXISTS public.track_activity_participation() CASCADE;

-- Recreate notify_event_participants function (this is for event_participants table)
...
-- Recreate notify_saved_event_participants function  
...
-- Recreate track_activity_participation function
...
CREATE TRIGGER notify_saved_participants
  AFTER INSERT OR UPDATE ON public.event_participants
  FOR EACH ROW
