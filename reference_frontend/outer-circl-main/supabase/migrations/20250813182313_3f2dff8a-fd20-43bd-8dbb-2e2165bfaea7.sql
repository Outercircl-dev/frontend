-- CRITICAL SECURITY FIXES (Check existing policies and fix conflicts)

-- First, check which policies exist and drop conflicting ones
DO $$
BEGIN
    -- Drop any existing conflicting policies
    DROP POLICY IF EXISTS "profiles_sensitive_owner_only" ON public.profiles_sensitive;
    DROP POLICY IF EXISTS "profiles_sensitive_rate_limited" ON public.profiles_sensitive;
...
-- Create strict owner-only policy for profiles_sensitive
CREATE POLICY "profiles_sensitive_strict_owner" 
...
CREATE POLICY "invitations_secure_own_email" 
...
CREATE POLICY "invitations_secure_admin" 
ON public.invitations 
FOR ALL 
USING (
  auth.uid() IS NOT NULL 
...
CREATE POLICY "payment_metadata_secure_owner" 
...
-- 4. UPDATE FUNCTION SECURITY
-- Update conversations function with proper security
...
-- 7. ADD SECURITY CONSTRAINTS (Skip if they already exist)
DO $$
BEGIN
    -- Add email format constraint
    ALTER TABLE public.invitations 
    ADD CONSTRAINT check_email_format_secure 
