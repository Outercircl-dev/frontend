...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendResponse = await fetch(
      `${API_URL.replace(/\/+$/, '')}/api/uploads/activity-image/presign`,
      {
        method: 'POST',
        headers: {
    ...
    if (!backendResponse.ok) {
      return NextResponse.json(
        isJson ? payload : { error: payload || 'Failed to generate upload URL' },
    ...
    console.error('Error in POST /rpc/v1/uploads/activity-image/presign', error)
