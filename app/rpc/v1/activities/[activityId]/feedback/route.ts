...
async function proxyToBackend(
  ...
  if (!API_URL) {
    console.error('API_URL is not configured for activity feedback')
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      isJson ? payload : { error: payload || 'Feedback request failed' },
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ activityId: string }> }) {
  try {
    ...
    return await proxyToBackend(`/api/activities/${activityId}/feedback/form`, token, { method: 'GET' })
  } catch (error) {
    if (error instanceof Error && error.name === 'AbortError') {
      return NextResponse.json(
        { error: 'Gateway Timeout', message: 'Feedback form request timed out' },
        { status: 504 },
      )
    }
    console.error('Error in GET /rpc/v1/activities/[activityId]/feedback:', error)
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ activityId: string }> }) {
  try {
    ...
    return await proxyToBackend(`/api/activities/${activityId}/feedback`, token, {
      method: 'POST',
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      return NextResponse.json(
        { error: 'Gateway Timeout', message: 'Feedback submission timed out' },
        { status: 504 },
      )
    }
    console.error('Error in POST /rpc/v1/activities/[activityId]/feedback:', error)
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 })
  }
}
